...
<script>
import { formatDate, dateStringToDate } from "@/util.js";
export default {
  props: {
    visible: Boolean,
    loading: Boolean,
  ...
  watch: {
    visible(prev, curr) {
      if (prev !== curr) {
        ...
        this.uploadedAt = formatDate(this.link.uploadedAt);
        this.description = this.link.description;
        this.isVR = this.link.isVR;
        this.checkUrlExist = false;
        this.findDownloadUrl = false;
      }
    }
  },
  methods: {
    submit() {
      this.$emit("submit", {
        url: this.url,
        uploadedAt: dateStringToDate(this.uploadedAt),
